...
{
    document.write(nNode.type + ": " + nNode.name + "<br />");
    document.write('<text'+String(level)+' style="padding-left:'+String(level*30)+'px; display:block;">')
    document.write("-------------------------------------<br />");
    for(var str in nNode.data)
        document.write(nNode.data[str].trim() + '<br />');
    document.write("-------------------------------------<br />");
    for(var i=0; i<nNode.children.length; i++)
    {
        print(nNode.children[i], level+1);
    }
    document.write('</text'+String(level)+'>')
}
